...
export function applyColorPalette(name, customHex, userId) {
  let palette;
  ...
  // 유저별 저장 + ThemeScript용 전역 키도 함께 갱신
  if (userId) {
    localStorage.setItem(`ts_color_${userId}`, name);
    if (name === 'custom' && customHex) localStorage.setItem(`ts_color_custom_${userId}`, customHex);
  }
  localStorage.setItem('ts_color', name);
  if (name === 'custom' && customHex) localStorage.setItem('ts_color_custom', customHex);
}

// 로그인 후 해당 유저의 색상 설정을 불러와 적용
export function loadUserColor(userId) {
  if (typeof window === 'undefined') return;
  const name = localStorage.getItem(`ts_color_${userId}`)
    || localStorage.getItem('ts_color')
    || 'indigo';
  const customHex = localStorage.getItem(`ts_color_custom_${userId}`)
    || localStorage.getItem('ts_color_custom')
    || '#4f46e5';
  applyColorPalette(name, name === 'custom' ? customHex : undefined, userId);
}
